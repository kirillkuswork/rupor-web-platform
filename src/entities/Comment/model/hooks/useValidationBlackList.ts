...
export function getCheckFormErrorsFn<TFieldValues extends FieldValues = FieldValues>(
  ...
  return async (data: { value: string, name: keyof TFieldValues }[]) => {
    const fields = data
      .filter(({ value }) => !!value)
    ...
    const { words: forbiddenWords = {} } = await store.dispatch(blacklistApi.endpoints.validateText.initiate({ fields })).unwrap();
...
export function useValidationBlacklist<TFieldValues extends FieldValues = FieldValues>({
  ...
  const checkError = useCallback(
    async (name: string, value: string) => checkErrors([{ name, value }]),
    [checkErrors],
  );
  ...
  return {
    ...
    handleChange,
    handleBlur,
    disabledSubmit: !!Object.values(errors).filter(
      (error) => error?.type !== INITIAL_BLACK_LIST_ERROR,
    ).length,
    loadingSubmit,
  };
}
